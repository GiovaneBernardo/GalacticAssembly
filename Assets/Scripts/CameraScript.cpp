...
namespace Plaza {
	...
	PlanetOctreeNode* FindLeafNodeAtPosition(PlanetOctreeNode* node, const glm::vec3& targetPos, float expectedSize,
											 float epsilon = 0.001f) {
		if (!node)
			return nullptr;

		glm::vec3 halfSizeVec = glm::vec3(node->mSize * 0.5f);
		glm::vec3 min = glm::vec3(node->mOrigin) - halfSizeVec;
		glm::vec3 max = glm::vec3(node->mOrigin) + halfSizeVec;

		bool inside = targetPos.x >= min.x - epsilon && targetPos.x <= max.x + epsilon &&
		...
		if (node->mIsLeaf) {
			...
			if (std::abs(node->mSize - expectedSize) < epsilon)
				return node;
			else
				return nullptr;
		}
		...
		for (auto& child : node->mChildren) {
			if (child) {
				PlanetOctreeNode* found = FindLeafNodeAtPosition(child.get(), targetPos, expectedSize, epsilon);
				if (found)
					return found;
			}
		}
	...
	std::vector<PlanetOctreeNode*> GetNeighborNodes(PlanetOctreeNode* current, PlanetOctreeNode* root) {
		std::vector<PlanetOctreeNode*> neighbors;

		if (!current || !root)
			return neighbors;

		const float epsilon = 0.001f;
		const glm::vec3 origin = current->mOrigin;
		const float size = current->mSize;
		...
		for (const glm::vec3& dir : directions) {
			...
			PlanetOctreeNode* found = FindLeafNodeAtPosition(root, neighborCenter, size, epsilon);
	...
	PlanetOctreeNode* GetNodeAtOffset(PlanetOctreeNode* node, const glm::ivec3& offsetDir, PlanetOctreeNode* root) {
		...
		glm::vec3 neighborOrigin = glm::vec3(node->mOrigin) + glm::vec3(offsetDir) * glm::vec3(node->mSize);

		// Traverse from root to find node with matching origin and same size
		std::function<PlanetOctreeNode*(PlanetOctreeNode*)> find = [&](PlanetOctreeNode* current) -> PlanetOctreeNode* {
			if (!current)
				return nullptr;

			float sizeDiff = fabs(current->mSize - node->mSize);
			float dist = glm::distance(glm::vec3(current->mOrigin), neighborOrigin);

			if (sizeDiff < EPSILON && dist < EPSILON && current->mIsLeaf)
				return current;

			if (!current->mIsLeaf) {
				for (auto& child : current->mChildren) {
					if (child) {
						PlanetOctreeNode* found = find(child.get());
						if (found)
							return found;
					}
				}
	...
	//
	//	bool ModifyScalarFieldInRadius(ScalarField& field, glm::ivec3 pos, float radius, float strength, DCOctreeNode*
	// node, uint64_t planetUuid) {
	//    bool modified = false;
	//
	...
	//            		DCOctreeNode* neighbor = static_cast<DCOctreeNode*>(GetNodeAtOffset(node, offset, root));
	//            		if (!neighbor) return;
	...
	//	PlanetOctreeNode* node,
	//	const glm::vec3& sphereCenter,
	//	float sphereRadius,
	//	std::vector<PlanetOctreeNode*>& outNodes)
	//	{
	//		if (!node) return;
	//
	...
	//			DCOctreeNode* hitNode = static_cast<DCOctreeNode*>(PlanetOctreeNode::TraverseRay(
	...
	//					*scene->GetComponent<TransformComponent>(scene->GetEntityByName("DebugNoCollider")->uuid),
	// scene, 					worldPosition);
	//
	//
	//
	...
	//				//glm::ivec3 scalarFieldPos = glm::ivec3(worldPosition - hitNode->origin + glm::vec3(hitNode->size
	/// 2.0f)); 				constexpr int scalarResolution = SCALAR_FIELD_SIZE; 				glm::vec3 corner =
	/// glm::vec3(hitNode->origin) -
	// glm::vec3(hitNode->size * 0.5f); 				float voxelSize = hitNode->size / float(scalarResolution);
	//
	//				if (mShowDebugEntities) {
	//					// Convert world position to scalar field index
	//					glm::vec3 local = (worldPosition - corner) / voxelSize;
	//					glm::ivec3 scalarFieldPos = glm::clamp(glm::ivec3(local), glm::ivec3(0),
	// glm::ivec3(scalarResolution - 1));
	//
	...
	//				std::vector<PlanetOctreeNode*> affectedNodes;
	...
	//					affectedNodes);
	//
	//				std::vector<DCOctreeNode*> nodesToUpdate;
	//				for (PlanetOctreeNode* node : affectedNodes) {
	//					DCOctreeNode* dcNode = static_cast<DCOctreeNode*>(node);
	//					// Transform worldPos into this node scalar field space
	//					glm::vec3 corner = glm::vec3(dcNode->origin) - glm::vec3(dcNode->size * 0.5f);
	//					float voxelSize = dcNode->size / float(SCALAR_FIELD_SIZE);
	//					glm::vec3 local = (worldPosition - corner) / voxelSize;
	//					glm::ivec3 scalarFieldPos = glm::clamp(glm::ivec3(local), glm::ivec3(0),
	// glm::ivec3(SCALAR_FIELD_SIZE));
	//
	//					bool modified = ModifyScalarFieldInRadius(dcNode->scalarField, scalarFieldPos, mDigRadius /
	//(dcNode->size / SCALAR_FIELD_SIZE), strength, dcNode, planet->uuid);
	//
	//					if (modified) {
	//						nodesToUpdate.push_back(dcNode);
	//					}
	//				}
	//
	...
	//
	//				for (DCOctreeNode* dcNode : nodesToUpdate) {
	//					PlanetGenerator::UpdateNodeMesh(scene, dcNode);
	//				}
	//			}
	//		}
	//	}
