...
namespace Plaza {
	...
	void PhysicalBodyScript::OnUpdate(Scene* scene) {
		...
		for (const auto& [key, body] : mBigPhysicalBodies) {
			...
			if (distance < 0.0001f)
				continue;
			float acceleration = (6.674e-11 * (glm::pow(10, 14)) * 1.0f) / (distance * distance);
			this->mLinearVelocity += glm::normalize(direction) * acceleration;
		}
		// Update the position with the velocity
		ECS::TransformSystem::SetWorldPosition(*scene->GetComponent<TransformComponent>(this->mEntityUuid), scene, scene->GetComponent<TransformComponent>(this->mEntityUuid)->GetWorldPosition() + mLinearVelocity);
